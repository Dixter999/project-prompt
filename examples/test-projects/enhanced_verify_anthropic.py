...
def check_environment():
    ...
    analyzer_path = os.path.join(os.path.dirname(OUTPUT_DIR), "analyze_with_anthropic_direct.py")
    if not os.path.exists(analyzer_path):
        print(f"{RED}❌ Analyzer script not found: {analyzer_path}{NC}")
        return False
...
def create_test_project(project_type):
    ...
    # Use the create_test_project.py script
    creator_script = os.path.join(OUTPUT_DIR, "create_test_project.py")
...
def run_anthropic_analysis(project_path, model="claude-3-haiku-20240307"):
    ...
    # Run analysis
    analyzer_script = os.path.join(os.path.dirname(OUTPUT_DIR), "analyze_with_anthropic_direct.py")
